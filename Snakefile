...
rule all:
    input:
        ...
        GENOME_FILE,
        # pre_processing
        expand(OUT_DIR + "/{sra_id}_filtered_1.fastq", sra_id=SRA_IDS),
        expand(OUT_DIR + "/{sra_id}_filtered_2.fastq", sra_id=SRA_IDS),
        # host
        expand(OUT_DIR + "/{sra_id}_{type}_reads.fastq.gz",
               sra_id=SRA_IDS, type=["host", "metagenome"]),
        # meta_genome
        expand(f"{OUT_DIR}/kraken2_{{sra_id}}.report", sra_id=SRA_IDS),
        expand(f"{OUT_DIR}/bracken_{{sra_id}}_output_{{level}}.report", 
               sra_id=SRA_IDS, level=LEVELS),
